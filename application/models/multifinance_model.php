...
public function get_all()
	...
	function kdotomatis() {
        $jenis = 'MLT'.date('ym');
        $query = $this->db->query("SELECT max(transaction_id) as maxID FROM multifinance WHERE transaction_id LIKE '$jenis%'");
	...
	function data(){
    $query =$this->db->query("SELECT * FROM product WHERE product='multifinance'");
       if ($query->num_rows() > 0) { 
